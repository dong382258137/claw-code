...
Use this when you are developing Claw Plus or testing a local checkout.

```powershell
git clone https://github.com/dong382258137/claw-code
Set-Location .\claw-code\rust
cargo build --workspace
.\target\debug\claw-plus.exe --help
...
.\target\release\claw-plus.exe --help
```
...
.\target\debug\claw-plus.exe --help
.\target\debug\claw-plus.exe doctor
.\target\debug\claw-plus.exe status --output-format json
.\target\debug\claw-plus.exe config --output-format json
```
...
Provider routing is model-prefix first. When multiple credentials exist, choose an explicit model prefix so `claw-plus` does not infer the wrong backend.
...
.\target\debug\claw-plus.exe --model "qwen-plus" prompt "reply with ready"
```
...
When moving between PowerShell and WSL, keep provider keys in the environment where `claw-plus` is actually running; Windows user env vars set with `setx` are not automatically the same as WSL shell exports.

## Troubleshooting checklist

- `claw-plus` not found: use `claw.exe` on Windows or run the binary by full path (`.\target\debug\claw-plus.exe`).
