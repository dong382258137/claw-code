...
await tools.external_browser_navigate({ url: "http://localhost:8080" });
await tools.external_browser_wait_for({ time: 2 });
await tools.external_browser_evaluate({ script: `document.querySelector(".btn").click()` });
await tools.external_browser_wait_for({ time: 2 });
const result = await tools.external_browser_evaluate({ script: `document.querySelector(".result").textContent` });
text(result);
```
...
await tools.external_browser_wait_for({ time: 2 });
const snap = await tools.external_browser_snapshot(); // get fresh refs
// parse snap → find new target ref
await tools.external_browser_click({ ref: "<new_ref>" });
await tools.external_browser_wait_for({ time: 2 });
