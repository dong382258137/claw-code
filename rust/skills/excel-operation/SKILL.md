...
| 类型 | 运算符 | 示例 |
|------|--------|------|
| 比较 | `=` `>` `<` `>=` `<=` `<>` | `金额 > 1000`、`工种 = '钢筋工'`、`状态 <> '已结清'` |
| 逻辑 | `AND` `OR` | `金额 > 1000 AND 工种 = '钢筋工'` |
| 模糊 | `LIKE` | `项目名称 LIKE '%挡土墙%'` |
| 空值 | `IS NULL` `IS NOT NULL` | `备注 IS NULL` |

#### 返回格式样例

```json
{
  "columns": ["序号", "费用类型", "金额", "备注"],
  "rows": [
    [1, "人工费", 90000, "5人×90天"],
    [2, "机械费", 90000, "2台×90天"]
  ],
...
| 错误信息 | 原因 | 解决方法 |
|---------|------|---------|
| `Sheet 'XXX' not found` | 工作表名错误 | 先调用 `list_excel_sheets` 确认工作表名 |
| `Column 'XXX' not found` | 列名拼写错误 | 先调用 `read_excel_file` 查看表头列名 |
| `Syntax error in query` | query 语法错误 | 检查运算符和引号，字符串值需用单引号 |
| `File not found` | 文件路径错误 | 使用绝对路径，注意双反斜杠 `\\` |
