...
description: "主动智能助手技能。提供每日简报生成、价格警报监控、系统健康检查、日程提醒等主动服务。当需要设置定时任务、监控条件、自动提醒时调用。"
---

# Proactive Assistant - 主动智能助手

## 核心功能

本技能为 TRAE IDE 提供主动智能能力，让 AI 能够：

1. **主动监控** - 持续监控价格、系统状态、文件变化
2. **定时任务** - 每日简报、定期报告、定时提醒
3. **条件触发** - 价格达到阈值、文件变化、时间到达
4. **智能提醒** - 基于上下文的主动提醒
...
- **desktop-vision**: 定时截图任务 → 视觉分析 → 保存分析结果
- **tradingview-analyzer**: 价格监控触发 → 自动分析图表 → 生成交易建议
...
1. **合理设置间隔** - 高频60-300秒，常规300-3600秒，定时用Cron
