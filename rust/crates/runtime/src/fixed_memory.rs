...
/// 快照 TTL(秒)。DeepSeek 官方**未公布**缓存 TTL(第三方实测口径不一:
/// 5 分钟刷新制 vs 几小时闲置清除制)。此值仅作为**无实测缓存状态时的
/// 保守兜底** —— 实测优先机制下,下一轮 `cache_read > 0`(上一轮命中)
/// 即无条件复用旧字节;只有上一轮实测未命中且距上次注入超过此值才重建。
pub const FIXED_MEMORY_TTL_SECS: i64 = 300;

/// 预留常量:原 LLM 前瞻触发窗口(270s = TTL 的 90%)。2026-08-31 起 LLM
/// 重写改由**保守冷启间隔**驱动(见 [`FIXED_MEMORY_COLD_THRESHOLD_MS`]),
/// 此常量不再参与触发判定,保留供测试构造"已超窗"快照与历史文档引用。
pub const FIXED_MEMORY_PRECEDING_WINDOW_MS: i64 = 270_000;

/// 保守冷启间隔(毫秒):距上次注入超过此值才认为缓存几乎必然已冷,触发
/// LLM 重写(把更新成本摊进"反正要全量重建"的冷启轮)。
///
/// 悖论修正(2026-08-31):**不能用"上一轮命中状态"做触发判据** —— 上一轮
/// 未命中恰恰说明它刚重建了缓存,本轮保持字节稳定即可命中,此时重写等于
/// 主动打碎刚建立的缓存(双重浪费)。正确判据是"本轮是否大概率冷启",
/// 唯一可测信号是距上次请求的间隔。取 30 分钟覆盖 DeepSeek 两种 TTL 口径
/// (5 分钟刷新制 / 几小时闲置清除制):两种口径下间隔 > 30 分钟都几乎必然
/// 已冷;TUI 长会话(间隔 < 30 分钟)则几乎不重写,前缀保持超稳定。
pub const FIXED_MEMORY_COLD_THRESHOLD_MS: i64 = 1_800_000;
...
/// 将固定记忆简报拆分为"稳定段(注入 messages 前缀)"与"易变段(丢弃)"。
///
/// 动机:简报的 `当前目标` / `下一步` 是**会话级状态**,却存在于工作区级
/// 快照中 —— 跨会话共享会残留旧任务目标(2026-08-31 实证:新会话每轮被注入
/// 上一任务的"当前目标"导致模型反复回答无关问题)。因此:
/// - **稳定段**(已完成项 / 历史教训 / 注脚)= 跨任务有效的工作区经验 → 注入
///   messages[0] 前缀(低频变化,命中缓存);
/// - **易变段**(当前目标 / 下一步)= 会话级任务目标 → **丢弃**,由尾部
///   Current Task 槽位从 `active_plan.task_summary` 提供(与会话绑定,无残留)。
///
/// 兼容两种简报格式:
/// - LLM 简报:`当前目标：...` / `下一步：...` 独立块;
...
///   `cache_hot` 表示上一轮请求实测命中缓存前缀(`cache_read > 0`)—— 这是
///   首要判据:命中即复用,即使已超固定 300s 兜底计时(A 修复:固定计时与
///   实际缓存活跃窗口脱钩;DeepSeek 官方未公布 TTL,见 [`FIXED_MEMORY_TTL_SECS`])。
