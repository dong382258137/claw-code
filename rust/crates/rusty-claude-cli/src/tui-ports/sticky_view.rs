//! Ported sticky header computation from grok-build (Apache-2.0).
//!
//! Source: xai-org/grok-build @ d02693a8
//! File:    crates/codegen/xai-grok-pager/src/scrollback/sticky.rs
//! Port scope: PromptDescriptor / RenderedPrompt / StickyHeaderLayout 类型、
//!             compute_sticky_layout(主算法)、calculate_render_height(内部 helper)、
//!             MIN_PINNED_HEIGHT / HEADER_CONTENT_GAP 常量、
//!             compute_claw_sticky_layout(claw 适配器,新增)
//!
//! Adaptation points:
//! - `if let ... && let ...` let chain (Rust 2024) → 嵌套 `if let` (Rust 2021 兼容)
//! - `pretty_assertions::assert_eq` (测试依赖) → 标准 `assert_eq!`
//! - 新增 `compute_claw_sticky_layout` adapter:转换 claw 滚动模型(距底部)→
//!   上游语义(距顶部),从显示行 breaks 构造 PromptDescriptor 列表
//!
//! 未移植(P2 集成阶段):
//! - app.rs 渲染循环改造(叠加 sticky header 区域 + 调整 visible_lines 裁剪)
//! - OutputEntry 增加 UserPrompt 变体(识别用户 prompt,PoC 阶段全部 entry 视为 sticky)
//!
//! # Layout Model
//!
...
/// claw 适配器:从 claw 的滚动状态计算 sticky header 布局。
///
/// claw 的滚动模型与上游相反:
/// - claw `scroll_offset = None`:跟随底部(看最新内容)
/// - claw `scroll_offset = Some(n)`:距底部 n 行(往回看历史)
/// - 上游 `scroll_offset`:距顶部 n 行(往下看内容)
///
/// 转换公式:`upstream_scroll = max_scroll - claw_offset`
/// - claw `None`(跟随底部)→ `upstream_scroll = max_scroll`(滚到最底)
/// - claw `Some(n)`(距底部 n 行)→ `upstream_scroll = max_scroll - n`
///
/// # 参数
/// - `claw_scroll_offset`:claw 的 scroll_offset(None=跟随底部,Some(n)=距底部 n 行)
/// - `max_scroll`:最大可滚动量 = 总显示行数 - 可视高度(负数视为 0)
/// - `display_breaks`:每个 entry 在 **wrap 后显示行空间** 的起始行号。
///   长度 = entries.len() + 1,display_breaks[0]=0,
///   display_breaks[i+1] = 前 i+1 个 entry 的显示行总数。
///   **注意**:必须用 wrap 后的显示行,不能用 output_view 的 cached_lines_breaks
///   (那是逻辑行)。由调用方(app.rs)从 wrapped_lines + entry 边界计算后传入。
/// - `viewport_height`:可视区域高度(行数)
///
/// # PoC 阶段限制
/// 当前所有 entry 都视为 sticky prompt。P2 集成时应改为只把用户 prompt entry
/// 标记为 sticky(需 OutputEntry 增加 UserPrompt 变体或 is_prompt 标记),
/// 避免 ToolCard/Thinking 等 entry 也被 pin 住。
pub fn compute_claw_sticky_layout(
    claw_scroll_offset: Option<usize>,
    max_scroll: usize,
    display_breaks: &[usize],
    viewport_height: u16,
) -> StickyHeaderLayout {
    // display_breaks 长度 = entries.len() + 1,至少需要 2(1 个 entry)才有意义
    if display_breaks.len() <= 1 {
        return StickyHeaderLayout::default();
    }

    // 转换滚动模型:claw(距底部)→ 上游(距顶部)
    let upstream_scroll = match claw_scroll_offset {
        None => max_scroll, // 跟随底部 = 滚到最底
        Some(offset) => max_scroll.saturating_sub(offset),
    };

    // 从 display_breaks 构造 PromptDescriptor 列表
    // windows(2) 产生相邻的 (start, end) 对,full_height = end - start
    let prompts: Vec<PromptDescriptor> = display_breaks
        .windows(2)
        .enumerate()
        .map(|(i, w)| {
            let y_virtual = w[0];
            // display_breaks 是 usize,行数可能超过 u16::MAX(超长会话),
            ...
            PromptDescriptor {
                ...
                full_height,
                min_height,
                sticky: true, // PoC: 全部 entry 视为 sticky
            }
        })
        .collect();

    compute_sticky_layout(upstream_scroll, viewport_height, &prompts)
}
...
mod tests {
    ...
    #[test]
    fn claw_adapter_empty_breaks_no_header() {
        // display_breaks 长度 <= 1(无 entry 或仅 [0])→ 无 header
        let layout = compute_claw_sticky_layout(None, 100, &[], 24);
        assert!(!layout.has_header());

        let layout = compute_claw_sticky_layout(None, 100, &[0], 24);
        assert!(!layout.has_header());
    }

    #[test]
    fn claw_adapter_follow_bottom_no_header() {
        // 跟随底部(claw None)且 max_scroll=0(内容不足一屏)→ 无 header
        // display_breaks: 1 个 entry,8 行,viewport=24 → max_scroll=0
        let breaks = vec![0, 8];
        let layout = compute_claw_sticky_layout(None, 0, &breaks, 24);
        assert!(!layout.has_header(), "内容不足一屏,无 header");
    }

    #[test]
    fn claw_adapter_follow_bottom_pinned_single_entry() {
        // 跟随底部(claw None),max_scroll > 0,单 entry → pinned
        // 单 entry 场景:无 next prompt,不会触发 push
        // breaks=[0, 20]:entry 0, y=0, full=20
        // claw None, max_scroll=6, viewport=24 → upstream_scroll=6
        // entry 0: scroll_past=6, render_height=20-6=14, min=4, min(14,24)=14
        // 无 next prompt → pinned entry 0, render_height=14
        let breaks = vec![0, 20];
        let layout = compute_claw_sticky_layout(None, 6, &breaks, 24);
        assert!(layout.has_header());
        assert!(layout.pinned.is_some());
        let pinned = layout.pinned.unwrap();
        assert_eq!(pinned.entry_idx, 0);
        assert_eq!(pinned.render_height, 14); // 20 - 6
    }

    #[test]
    fn claw_adapter_scroll_up_pinned_single_entry() {
        // claw scroll_offset = Some(n) 表示距底部 n 行(往回看)
        // 单 entry 场景:breaks=[0, 8], entry 0, y=0, full=8
        // claw Some(5), max_scroll=6, viewport=10 → upstream_scroll = 6-5 = 1
        // entry 0: scroll_past=1, render_height=8-1=7, min=4, min(7,10)=7
        // 无 next prompt → pinned entry 0, render_height=7
        let breaks = vec![0, 8];
        let layout = compute_claw_sticky_layout(Some(5), 6, &breaks, 10);
        assert!(layout.pinned.is_some());
        let pinned = layout.pinned.unwrap();
        assert_eq!(pinned.entry_idx, 0);
        assert_eq!(pinned.render_height, 7); // 8 - 1
    ...
    #[test]
    fn claw_adapter_adjacent_entries_trigger_push() {
        // claw 的 display_breaks 连续(相邻 entry 无 gap),多 entry 场景
        // 下一个 entry 总会触发 push(因为 next_naive_row = entry0.full - scroll,
        // header_with_gap = render_height + 1 = next_naive_row + 1,所以
        // next_naive_row < header_with_gap 恒成立)。
        //
        // breaks=[0, 8, 16]:entry 0(y=0,full=8), entry 1(y=8,full=8)
        // claw None, max_scroll=6, viewport=10 → upstream_scroll=6
        // entry 0: pinned_idx=0, render_height=8-6=2, clamped to min=4
        // entry 1: next_naive_row=8-6=2, header_with_gap=4+1=5, 2<=5 → push!
        // pushed_visible=2-1=1, pushed_render_height=min(8,4)=4, push_clip=4-1=3
        let breaks = vec![0, 8, 16];
        let layout = compute_claw_sticky_layout(None, 6, &breaks, 10);
        assert!(layout.has_header());
        assert!(layout.pushed.is_some(), "相邻 entry 应触发 push");
        assert!(layout.pinned.is_none(), "push 状态下 pinned 为 None");
        let pushed = layout.pushed.unwrap();
        assert_eq!(pushed.entry_idx, 0);
        assert_eq!(pushed.render_height, 4); // min(8, 4)
        assert_eq!(pushed.visible_height(), 1); // 2 - 1 = 1
    }

    #[test]
    fn claw_adapter_scroll_model_conversion() {
        // 验证滚动模型转换:claw None == claw Some(0) == 上游 max_scroll
        // 单 entry 避免相邻 push 干扰
        let breaks = vec![0, 20];

        // claw None(跟随底部)→ 上游 max_scroll
        let layout_none = compute_claw_sticky_layout(None, 6, &breaks, 24);

        // claw Some(0)(距底部 0 行 = 跟随底部)→ 上游 max_scroll - 0 = max_scroll
        let layout_zero = compute_claw_sticky_layout(Some(0), 6, &breaks, 24);

        // 两者应产生相同布局
        assert_eq!(layout_none, layout_zero);
    }

    #[test]
    fn claw_adapter_min_height_floored_to_1() {
        // entry 高度为 1(full_height=1)→ min_height = min(4, 1) = 1
        // 单 entry:breaks=[0, 1], claw None, max_scroll=6, viewport=24
        // upstream_scroll=6, entry 0: scroll_past=6, render_height=1-6=0(saturating),
        // clamped to max(1, 1)=1, min(1, 24)=1
        let breaks = vec![0, 1];
        let layout = compute_claw_sticky_layout(None, 6, &breaks, 24);
        if let Some(pinned) = layout.pinned {
            assert_eq!(pinned.entry_idx, 0);
            assert_eq!(pinned.render_height, 1, "render_height 应 floor 到 1");
        } else if let Some(pushed) = layout.pushed {
            // 单 entry 不会有 push,但防御性检查
            assert!(pushed.render_height >= 1, "render_height 应至少 1");
        }
    }
}
