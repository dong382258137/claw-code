...
const PASTE_FOLD_LINE_THRESHOLD: usize = 3;

/// paste-cache 根目录：`%USERPROFILE%\.claw\paste-cache\`。
/// 与 sessions 目录平级，存放超阈值粘贴的原始内容。
fn paste_cache_root() -> Option<PathBuf> {
...
fn paste_cache_path(session_id: &str, paste_id: u32) -> Option<PathBuf> {
...
fn pasted_text_ref_num_lines(text: &str) -> usize {
...
/// 触发条件：字符数 > 阈值 **或** 行数 > 阈值。
fn should_fold_paste(input: &str) -> bool {
...
fn store_paste_and_make_placeholder(
    input: &str,
    session_id: &str,
    paste_id: u32,
) -> String {
...
fn expand_paste_placeholders(input: &str, session_id: &str) -> String {
